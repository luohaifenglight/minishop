...
def get_full_path(uri):
    return "https://i1.hemaweidian.com/" + uri
...
class AliyunBaseStorage(BucketOperationMixin, Storage):
    ...
    def __init__(self):
        self.access_key_id = self._get_config('OSS_ACCESS_KEY')
        self.access_key_secret = self._get_config('OSS_ACCESS_SECRET')
    ...
    def _save(self, name, content):
        ...
        # is_encrypt = getattr(content, "is_encrypt", 0)
        # if is_encrypt:
        #     from utils.encrypt import AESCommon
        #     content_str = AESCommon.encrypt(content_str)
    ...
    def url(self, name):
        ...
        # 没办法了。直接换url，用cname会超时
        return urllib.unquote(self.bucket._make_url(self.bucket_name, name) + settings.STYLE_NAME).replace(
            settings.END_POINT_URL, settings.ALIYUN_OSS_CNAME_URL)
